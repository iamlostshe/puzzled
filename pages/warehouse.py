"""Модуль для работы с страницей склада."""

import flet as ft


def edit_warehouse_page(page: ft.Page) -> ft.SafeArea:
    """Изменение страницы работы со складом."""
    return ft.SafeArea(page)


def warehouse_page(page: ft.Page) -> ft.SafeArea:
    """Страница работы со складом."""

    def warehouse_page_on_click(
        e: ft.core.control_event.ControlEvent,  # noqa: ARG001
        page: ft.Page,
    ) -> None:
        """Нажатие на кнопку '+'."""
        page.snack_bar = ft.SnackBar(ft.Text("Этот раздел пока в разработке..."))
        page.snack_bar.open = True
        page.update()

    plus_button = ft.FloatingActionButton(
        icon=ft.Icons.ADD,
        on_click=lambda e: warehouse_page_on_click(e, page),
    )

    return ft.SafeArea(
        ft.Column(
            width=page.width,
            controls=[
                ft.Row(
                    controls=[
                        ft.TextField(label="Поиск отчётов", expand=True),
                        plus_button,
                    ],
                ),
            ],
        ),
    )
