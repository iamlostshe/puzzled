...
def reports_page(page: ft.Page) -> ft.SafeArea:
    ...
    def reports_page_on_click(e, page: ft.Page) -> None:
        'Нажатие на кнопку "+"'
    ...
    def get_table() -> ft.Row | ft.DataTable:
        ...
        if len(items) == 0:
            ...
            # Подписываем колонки
            for column_name in (
                '№', 'Вид отчёта', 'Формат отчёта', 'Дата и время'
            ):
    ...
    plus_button = ft.FloatingActionButton(icon=ft.Icons.ADD, on_click=lambda e: reports_page_on_click(e, page))
